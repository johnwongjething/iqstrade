#!/usr/bin/env python3
"""
Email System Diagnostic Tool - Fixed Version
Works with existing config structure
"""
...
def check_environment_variables():
    ...
    # Check variables that the config.py actually uses
    config_vars = [
    ...
    if missing_vars:
        ...
        print("\n📋 You need to create a .env.local file in the backend directory with:")
...
def check_database_connection():
    ...
    try:
        ...
        if emails_table_exists:
            ...
            required_columns = ['id', 'subject', 'body', 'from_address', 'attachments', 'processed_for_payments']
...
def check_openai_integration():
    ...
    try:
        ...
        if api_key:
            ...
            # Test with a simple call using the new API
            try:
...
def generate_fix_recommendations():
    ...
    recommendations = [
        "1. **Environment Variables**: Create .env.local file in backend directory",
        "2. **Database**: Run migrations to create missing tables/columns",
        "3. **Email Scheduler**: Start email scheduler with: python email_scheduler.py",
        "4. **Test Emails**: Use new complex email templates with real BL numbers",
        "5. **Monitoring**: Check email_scheduler.log for errors"
    ]
...
def main():
    ...
    print("🚀 Email System Diagnostic Tool - Fixed Version")
    print("=" * 60)
    ...
    if passed == len(checks):
        ...
        print("2. Test with new complex email templates")
        print("3. Monitor email_scheduler.log for any issues")
    else:
