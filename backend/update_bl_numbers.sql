-- PostgreSQL script to update existing emails with BL numbers
...
-- Update emails with extracted BL numbers
DO $$
DECLARE
    email_record RECORD;
    bl_numbers TEXT[];
...
BEGIN
    ...
    LOOP
        ...
        bl_numbers := extract_bl_numbers(
            COALESCE(email_record.subject, '') || ' ' || COALESCE(email_record.body, '')
        );
        
        -- Update the email with extracted BL numbers
        UPDATE customer_emails 
        SET bl_numbers = bl_numbers 
        WHERE id = email_record.id;
        
        updated_count := updated_count + 1;
        
        -- Print progress (optional)
        IF array_length(bl_numbers, 1) > 0 THEN
            RAISE NOTICE 'Updated email %: BL numbers = %', email_record.id, bl_numbers;
        ELSE
