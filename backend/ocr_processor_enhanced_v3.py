#!/usr/bin/env python3
"""
Enhanced OCR Processor V3 - Improved Heavy Regex Fallback
Fixes the regex patterns that failed in V2 and adds better fallback mechanisms
"""
...
class EnhancedAIOCRProcessorV3:
    """Enhanced AI-based OCR processor with improved heavy regex fallback"""
    ...
    def extract_fields_openai_enhanced_v3(self, pdf_path: str) -> Dict:
        """Enhanced AI-based field extraction with improved heavy regex fallback"""
        ...
        if not ai_success or not has_meaningful_text:
            logger.info("AI extraction insufficient, forcing improved heavy regex extraction")
            return self._force_improved_regex_extraction(pdf_path)
        ...
        logger.info(f"Enhanced AI OCR V3 completed with confidence: {overall_confidence:.2f}")
        return enhanced_fields

    def _force_improved_regex_extraction(self, pdf_path: str) -> Dict:
        """Improved heavy regex extraction with better patterns and fallbacks"""
        logger.info("Starting improved heavy regex extraction")
        ...
        try:
            ...
            # Detect document format for better pattern matching
            document_format = self._detect_document_format(text)
            logger.info(f"Detected document format: {document_format}")
            
            # Get format-specific patterns
            patterns = self._get_format_specific_patterns(document_format, text)
            ...
            result = {
                ...
                'pricing_method': fee_calculation['pricing_method'],
                'ocr_confidence_score': 0.7,  # Higher confidence for improved regex
                'pricing_calculation_log': fee_calculation['calculation_details'],
                'raw_text': text,
            ...
            logger.info(f"Improved heavy regex extraction completed with {len(extracted_fields)} fields")
            return result
            
        except Exception as e:
            logger.error(f"Improved heavy regex extraction failed: {e}")
            ...
            return {
                ...
                'pricing_calculation_log': {'method': 'default', 'reason': 'Improved regex extraction failed'},
                'raw_text': '',
                'extraction_method': 'failed'
            }

    def _detect_document_format(self, text: str) -> str:
        """Detect the document format for better pattern matching"""
    ...
    def _get_format_specific_patterns(self, document_format: str, text: str) -> Dict:
        """Get format-specific regex patterns"""
        
        # Base patterns that work for most formats
        base_patterns = {
            'shipper': [
        ...
        if document_format == 'maersk':
            # MAERSK specific patterns
            base_patterns['bl_number'].insert(0, r'B/L\s+NO[:\s]*([A-Z0-9]+)')
            base_patterns['vessel'].insert(0, r'VESSEL[:\s]*([A-Z\s0-9]+)')
            base_patterns['port_of_loading'].insert(0, r'PORT\s+OF\s+LOADING[:\s]*([A-Z\s]+)')
            base_patterns['port_of_discharge'].insert(0, r'PORT\s+OF\s+DISCHARGE[:\s]*([A-Z\s]+)')
        
        elif document_format == 'oocl':
            # OOCL specific patterns
            base_patterns['bl_number'].insert(0, r'B/L\s+NUMBER[:\s]*([A-Z0-9]+)')
            base_patterns['vessel'].insert(0, r'EXPORTING\s+CARRIER[:\s]*([A-Z\s0-9]+)')
        
        elif document_format == 'awb':
            # Air Waybill specific patterns
            base_patterns['shipper'].insert(0, r'SHIPPER\'S\s+NAME[:\s]*([A-Z\s&\.]+(?:LTD|INC|LLC|CORP|CO|COMPANY)?)')
            base_patterns['consignee'].insert(0, r'CONSIGNEE\'S\s+NAME[:\s]*([A-Z\s&\.]+(?:LTD|INC|LLC|CORP|CO|COMPANY)?)')
            base_patterns['flight_or_vessel'].insert(0, r'CARRIER[:\s]*([A-Z\s0-9]+)')
            base_patterns['port_of_loading'].insert(0, r'AIRPORT\s+OF\s+DEPARTURE[:\s]*([A-Z\s]+)')
            base_patterns['port_of_discharge'].insert(0, r'AIRPORT\s+OF\s+DESTINATION[:\s]*([A-Z\s]+)')
...
# Global instance
enhanced_ai_ocr_v3 = EnhancedAIOCRProcessorV3()

def extract_fields_openai_enhanced_v3(pdf_path: str) -> Dict:
    """Enhanced AI-based field extraction V3 with improved heavy regex fallback"""
    return enhanced_ai_ocr_v3.extract_fields_openai_enhanced_v3(pdf_path)

if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: python ocr_processor_enhanced_v3.py <pdf_path>")
        sys.exit(1)
    
    result = extract_fields_openai_enhanced_v3(sys.argv[1])
