...
def save_draft_reply(to_addr, subject, reply, confidence_result=None):
    ...
    if row:
        ...
        cur.execute("INSERT INTO customer_emails (sender, subject, body, created_at) VALUES (%s, %s, %s, %s) RETURNING id", (to_addr, subject, '', get_hk_now()))
        ...
        for num in email_ids:
            try:
                ...
                try:
                    ...
                    cursor.execute(
                        """
                        INSERT INTO customer_emails (sender, subject, body, created_at, processed_for_payments, message_id, attachments, bl_numbers) 
                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                        ON CONFLICT (message_id) DO NOTHING
                        RETURNING id;
                        """,
                        (from_addr, subject, body, get_hk_now(), False, message_id, attachment_json, [])
                    )
                    result = cursor.fetchone()
