#!/usr/bin/env python3
"""
Local PDF extraction test - no API required
Uses PyPDF2 or similar to extract text locally
"""
...
def test_pdf_extraction(pdf_path: str) -> dict:
    ...
    try:
        ...
        print(f"  Regex Processing: {regex_time:.2f}s")
        print(f"  Box Processing: {box_time:.2f}s")
        print(f"  Total Regex: {extract_time + regex_time:.2f}s")
        print(f"  Total Box: {extract_time + box_time:.2f}s")
...
def analyze_results(all_results: list):
    ...
    if successful_tests:
        ...
        total_extract_time = sum(r["extract_time"] for r in successful_tests)
        ...
        print(f"  Total Text Extraction: {total_extract_time:.2f}s")
...
def main():
    """Main test function"""
    print("🧪 LOCAL PDF EXTRACTION TEST")
    print("="*60)
    print("Testing both regex and box-based approaches on real PDFs (no API required)")
    ...
    print(f"\n💾 Detailed results saved to: test_results_local.json")
