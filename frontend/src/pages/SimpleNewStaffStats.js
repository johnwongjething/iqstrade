...
const SimpleNewStaffStats = ({ t = x => x }) => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/balance/search?q=${encodeURIComponent(searchTerm)}`, {
        credentials: 'include'
      });
  ...
  const loadCustomerBalance = async (username) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/balance/${username}`, {
        credentials: 'include'
      });
  ...
  const loadTransactionHistory = async (username) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/balance/${username}/history`, {
        credentials: 'include'
      });
  ...
  const handleAdjustment = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/balance/${selectedCustomer.username}/adjust`, {
        method: 'POST',
        headers: {
      ...
      const data = await response.json();
              setSuccess(`Balance adjusted successfully. New balance: $${data.new_balance?.toFixed(2) || '0.00'}`);
              ...
              <CardContent>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                  <Typography variant="h4" sx={{ mr: 2 }}>
                    {formatCurrency(customerBalance.balance_amount || customerBalance.balance || 0)}
                  </Typography>
                  <Chip
                                    label={(customerBalance.balance_amount || customerBalance.balance || 0) >= 0 ? 'Credit' : 'Debit'}
                color={(customerBalance.balance_amount || customerBalance.balance || 0) >= 0 ? 'success' : 'error'}
                  />
                </Box>
                <Typography variant="body2" color="text.secondary">
                  Last Updated: {customerBalance.last_updated ? formatDate(customerBalance.last_updated) : 'N/A'}
                </Typography>
...
export default SimpleNewStaffStats; 