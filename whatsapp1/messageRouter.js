  ...
  client.ev.on('messages.upsert', async (msgEvent) => {
    ...
    for (const msg of messages) {
      ...
      try {
        ...
        // Prepare context with combined BL numbers and payment amounts from both text and PDF
        let context = {};
        if (pdfFields && Object.keys(pdfFields).length > 0) {
          context = {
            bl_numbers: pdfFields.bl_numbers || [],
            paid_amount: pdfFields.paid_amount || null,
            raw_text: pdfFields.raw_text || '',
            has_pdf: true
          };
        }
        
        // Always process through the same route - combine text and PDF content
        let messageToProcess = text;
        if (pdfFields && Object.keys(pdfFields).length > 0) {
          // If we have PDF content but no text, use the PDF raw text as the message
          if (!text || text.trim() === '') {
            messageToProcess = pdfFields.raw_text || '';
          }
          // If we have both text and PDF, combine them
          else if (pdfFields.raw_text) {
        ...
        console.log('[DEBUG] Processing message:', { 
          originalText: text, 
          messageToProcess: messageToProcess.substring(0, 200) + '...', 
          hasPdf: !!pdfFields,
          pdfBLs: pdfFields?.bl_numbers || [],
          pdfAmount: pdfFields?.paid_amount || null
        });
        
        let reply = await escalationWrapper(chatHandler, messageToProcess, sender, context, client);
