...
async function checkPaymentProcessed(blId, paymentSource) {
  try {
    console.log(`[DEBUG] checkPaymentProcessed called - BL ID: ${blId}, Source: ${paymentSource}`);
    
    const result = await pool.query(
      'SELECT COUNT(*) FROM customer_balance_transactions WHERE reference_id = $1 AND payment_source = $2',
      [blId, paymentSource]
    );
    
    const count = parseInt(result.rows[0].count);
    console.log(`[DEBUG] Found ${count} existing payment records for BL ${blId} from ${paymentSource}`);
...
async function markPaymentProcessed(blId, paymentSource, createdBy) {
  try {
    console.log(`[DEBUG] markPaymentProcessed called - BL ID: ${blId}, Source: ${paymentSource}, Created By: ${createdBy}`);
    ...
    let username = null;
    if (customerResult.rows.length > 0 && customerResult.rows[0].customer_username) {
      username = customerResult.rows[0].customer_username;
    }
    
    // If no customer_username found, try to create a system user or use admin
    if (!username) {
      ...
      if (adminResult.rows.length > 0) {
        ...
        // If admin doesn't exist, skip the transaction record to avoid foreign key error
        console.log(`No valid username found for BL ${blId}, skipping transaction record`);
        return;
      }
    }
    
    // Verify the username exists in users table before inserting
    ...
    if (userCheck.rows.length === 0) {
      console.log(`Username ${username} not found in users table, skipping transaction record for BL ${blId}`);
      return;
    }
    
    console.log(`[DEBUG] Inserting payment processed record - Username: ${username}, BL ID: ${blId}, Source: ${paymentSource}`);
    
    await pool.query(
      'INSERT INTO customer_balance_transactions (username, transaction_type, amount, reference_type, reference_id, payment_source, description, created_by) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
      [username, 'credit', 0, 'bill_of_lading', blId, paymentSource, `Payment marked as processed for BL ${blId}`, createdBy]
    );
    console.log(`Payment marked as processed for BL ${blId} from ${paymentSource} using username ${username}`);
  } catch (error) {
...
async function processPaymentBalance(username, paymentAmount, invoiceAmount, blId, paymentSource, createdBy) {
  try {
    // First verify the username exists in users table
    const userCheck = await pool.query(
      'SELECT username FROM users WHERE username = $1',
      [username]
    );
    ...
    if (userCheck.rows.length === 0) {
      console.log(`Username ${username} not found in users table, skipping balance processing for BL ${blId}`);
      return 0;
    }
...
async function getOriginalPaymentDate(blId, paymentSource) {
  try {
    const result = await pool.query(
      'SELECT created_at FROM customer_balance_transactions WHERE reference_id = $1 AND payment_source = $2 ORDER BY created_at DESC LIMIT 1',
      [blId, paymentSource]
    );
