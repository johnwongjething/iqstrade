...
function extractBLNumbers(text) {
  // Use the same comprehensive pattern as the backend
  const blPattern = /(?:提单号[:：]?\s*)?(BL-\d{4,}|\d{3,}-\d{3,}|\d{6,}|[A-Z]{2,4}\d{2,})(?![^@]*@)/gi; // Exclude strings with @
  ...
  // Filter out common bank reference patterns (same as backend)
  const bankRefPatterns = ['TEST', 'REF', 'BANK', 'PAY', 'TRANS', 'TXN'];
  const filteredMatches = matches.filter(bl => {
    const blUpper = bl.toUpperCase();
    return !bankRefPatterns.some(prefix => blUpper.startsWith(prefix));
  });
  
  const filteredExtraBLs = extraBLs.filter(bl => {
    const blUpper = bl.toUpperCase();
    return !bankRefPatterns.some(prefix => blUpper.startsWith(prefix));
  });
  
  return Array.from(new Set([...filteredMatches, ...filteredExtraBLs]));
}
...
async function chatHandler(message, sender, context = {}) {
  ...
  if (blNumbersInMessage.length > 0 && /how much|pricing|cost|fee|price|invoice/.test(lowerMsg)) {
    intent = 'ask_pricing';
  ...
  if ((wantsInvoice && wantsCTN) && (validBLs.length > 0 || conversationHistory[sender].session.pendingBLs.length > 0) && conversationHistory[sender].session.verifiedEmail) {
    ...
    if (invoiceSum > 0) {
      if (Math.abs(diff) < 0.01 || diff > 0) {
        ...
        try {
          const pdfPath = await generateReceiptPDF({ blNumbers: cleanValidBLs, paidAmount, invoiceDetails, customerName: '' });
          const receiptUrl = await uploadReceiptToCloudinary(pdfPath);
          await updateBLStatusAndReceipt(cleanValidBLs, receiptUrl);
          fs.unlink(pdfPath, () => {});
      ...
      replyLines.push(`We detected a payment of $${paidAmount} for BL number(s): ${cleanValidBLs.join(', ')}. If you need a receipt, let us know.`);
    }
      ...
      try {
        ...
        if (invoiceInfo.length > 0) {
          ...
          for (const info of invoiceInfo) {
            ...
            pricingDetails.push(`BL ${info.bl_number}: CTN Fee $${ctnFee}, Service Fee $${serviceFee}`);
          }
          
          const totalFee = totalCTNFee + totalServiceFee;
          ...
          if (invoiceInfo.length === 1) {
            replyLines.push(`For BL ${invoiceInfo[0].bl_number}:\n• CTN Fee: $${Number(invoiceInfo[0].ctn_fee) || 0}\n• Service Fee: $${Number(invoiceInfo[0].service_fee) || 0}\n• Total: $${totalFee}`);
          } else {
