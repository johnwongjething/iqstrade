...
// Use /tmp for Railway, fallback to local for dev
const AUTH_INFO_PATH = process.env.AUTH_INFO_PATH || (process.env.RAILWAY_STATIC_URL ? '/tmp/auth_info' : path.join(__dirname, 'auth_info'));
const LOGS_PATH = process.env.LOGS_PATH || (process.env.RAILWAY_STATIC_URL ? '/tmp/logs' : path.join(__dirname, 'logs'));

// Ensure folders exist
if (!fs.existsSync(AUTH_INFO_PATH)) {
  fs.mkdirSync(AUTH_INFO_PATH, { recursive: true });
  console.log('Created auth_info folder at', AUTH_INFO_PATH);
}
if (!fs.existsSync(LOGS_PATH)) {
  fs.mkdirSync(LOGS_PATH, { recursive: true });
  console.log('Created logs folder at', LOGS_PATH);
}
process.env.AUTH_INFO_PATH = AUTH_INFO_PATH;
process.env.LOGS_PATH = LOGS_PATH;
